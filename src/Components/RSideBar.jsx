...
const RSideBar = () => {
  ...
  const [updateWidth, setUpdateWidth] = useState(15);
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      setUpdateWidth((width) => width - 0.25);
    }, 1000);
  ...
  const dynamicWidth = `${updateWidth}rem`;
  return (
    <>
      <div className="w-[50%] h-[85%] rounded-2xl  flex flex-col text-white border-1 p-4 border-[rgba(79,209,199,0.3)]">
        ...
        <div className="p-6">
          ...
          <div
            ...
            // className={` w-[${time / 10}rem] h-[1rem] bg-amber-400`}
          ></div>
        </div>
        <div>
          {hints.map((item) => {
            return (
              <div
                key={item.id}
                className="p-4 mb-4 bg-[rgba(63,22,65,0.3)] rounded-xl border-[1px] border-[rgba(226,82,234,0.3)]"
              >
